...
| | noctty | Windows Terminal |
|---|---|---|
| **キー入力 → 直後のフレームが画面に出るまで**（中央値） | **8.2 ms** | **23.6 ms** |
| 同 実測レンジ | 4.9〜11.4 ms | 21.1〜25.4 ms |
| **アイドル 10 秒の表示フレーム数** | **0** | 18 |
| 同 GPU 実働 | **0 ms** | 18.7 ms |
| コールドスタート（各セッションの中央値の範囲、3 セッション） | 481〜574 ms | 467〜492 ms |

1 行目は **そのフレームがエコー文字を含むことを検証していない近似値**です。詳細は後述します。

**入力直後に出たフレームが画面に到達するまでの時間は noctty が約 2.9 倍短く、分布が全く重なりません。** 差は約 15 ms で、60 Hz の 1 フレーム（16.7 ms）に近い量です。

ただしこれは「打鍵した文字が見えるまで」そのものではありません。拾っているのは入力後の最初のフレームで、そこにエコー文字が含まれることは確認していません。noctty 公式のハーネスにある `key_to_first_swap_ms_proxy` は、受理するスワップにエコー出力の世代が含まれることまで要求したうえで、なお `photon_measurement: false`（光子を測ってはいない）と記録しています。今回の計測はそれより弱い近似です。

**アイドル時、noctty は 1 フレームも描きません。** Windows Terminal は毎秒 1.8 枚出し続けます。ただし絶対値は 10 秒あたり GPU 18.7 ms（0.19%）と小さく、速さではなく電力の話です。

### コールドスタートは互角

差より大きいものが見つかりました。再起動を挟んで 3 セッション測ったところ、**同一バイナリでセッションごとの平均が 87 ms 動きました**（483 → 570 → 566 ms）。ターミナル間の差（14〜87 ms）はこの変動に収まります。
...
- 入力直後の描画の速さを重視する（上記の非公式計測で差が最も大きかった項目。ただし近似値）
- テレメトリを一切通したくない
- CLI からの構造化された自動化が要る
- Ghostty の設定文法とテーマ資産をそのまま使いたい
...
- **非公式に測った範囲では、入力直後のフレームが画面に出るまでは noctty が約 2.9 倍短く、起動は互角**。ただし近似値であり、スループットは測れていない
